...
cm_key_file = os.path.join(truth_dir, 'in_the_wild_labels.txt')
...
def eval_to_score_file(score_file, cm_key_file):
    ...
    #original: cm_scores = submission_scores.merge(cm_data, left_on=0, right_on=1, how='inner')  # check here for progress vs eval set
    #claude:
    submission_scores[0] = submission_scores[0].astype(str)
    cm_data[1] = cm_data[1].astype(str)
    submission_scores[0] = submission_scores[0].astype(str)
    cm_data[1] = cm_data[1].astype(str)
    # Ajoute ceci AVANT le merge dans evaluate_in_the_wild.py
